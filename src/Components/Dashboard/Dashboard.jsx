...
const Dashboard = () => {
  ...
  const filterchecklist = useSelector(
    (store) => store.checklist.filterchecklist
  );
  ...
  useEffect(() => {
    ...
    axios
      .get(commonapiurl + "checklist/getchecklist/" + userid + "?time=" + query)
      .then((response) => {
          ...
          tempchecklist.map((checklist) => {
            if (checklist.duedate === "")
              return {
                ...checklist,
                tempname:
                  checklist.name.length < 50
                    ? checklist.name
                    : checklist.name.slice(0, 50) + "...",
              };
            return {
              ...checklist,
              ...
              tempname:
                checklist.name.length < 50
                  ...
                  : checklist.name.slice(0, 50) + "...",
              duedatestring:
                montharr[parseInt(checklist.duedate.split("-")[1]) - 1] +
                " " +
                checklist.duedate.split("-")[2] +
                "th",
            };
          })
        );
  ...
  }, [checklistchanged, filterchecklist, userid]);
  function handlesectiontype(typeval, checklistid) {
    setshowchecklistobj(
      Object.keys(showchecklistobj)
        .filter((objKey) => objKey !== checklistid)
        .reduce((newObj, key) => {
    ...
    setallchecklist(
      allchecklist.map((checklistobj) => {
        if (checklistid === checklistobj._id)
          return { ...checklistobj, sectiontype: typeval };
        else return checklistobj;
      })
    );
    axios
      .patch(commonapiurl + "checklist/updatechecklist/" + checklistid, {
        sectiontype: typeval,
      })
      .then((response) => {
    ...
    setallchecklist(
      allchecklist.map((checklistobj) => {
        if (checklistid === checklistobj._id) {
          ...
          tempchecklistarr = checklistobj.checklistarr.map(
            (checklisttemp, i) => {
              if (i === index) {
  ...
  function handleaddkey(checklistid, sectiontype) {
    ...
    setshowchecklistobj({
      ...showchecklistobj,
      [checklistid]: sectiontype,
    });
  }
  function collapsechecklist(sectiontype) {
    setshowchecklistobj(
      Object.keys(showchecklistobj)
        .filter((objKey) => showchecklistobj[objKey] !== sectiontype)
        .reduce((newObj, key) => {
          newObj[key] = showchecklistobj[key];
          return newObj;
        }, {})
    ...
    <>
      <div className={styles.maincontainer} onClick={closepopupcontainer}>
        ...
        <div className={styles.titledate}>
          <p>Board</p>
          <p onClick={() => sethandlepopupcard(!handlepopupcard)}>
            {filterchecklist} <img src={downarrowimg} alt="" />
          </p>
          {handlepopupcard && (
            <div>
                ...
                return checklistobj.sectiontype === "backlog" ? (
                  <div className={styles.sectioncard}>
                    <div className={styles.priority}>
                      <div>
                        {checklistobj.priority === "high" && (
                          <img src={Ellipseimg} />
                        )}
                        {checklistobj.priority === "moderate" && (
                          <img src={Ellipse2img} />
                        )}
                        {checklistobj.priority === "low" && (
                          <img src={Ellipse3img} />
                        )}
                        {checklistobj.priority === "high" && (
                          <p>HIGH PRIORITY</p>
                      ...
                      <div
                        className={styles.threedotclass}
                      ...
                      {checklistobj._id === openpopup && (
                        <div className={styles.popupcard}>
                          <p onClick={() => Editchecklist(checklistobj)}>
                            Edit
                          </p>
                          <CopyToClipboard
                            text={
                              url + "/readonlychecklist/" + checklistobj._id
                            }
                          >
                            <p onClick={() => handleshare(checklistobj._id)}>
                              Share
                            </p>
                          </CopyToClipboard>
                          <p
                            onClick={() =>
                              handledeletechecklist(checklistobj._id)
                            }
                          >
                            Delete
                          </p>
                    ...
                    <div className={styles.checklistclass}>
                      <p>
                        Checklist{" "}
                        {`(${checklistobj.markedval}/${checklistobj.checklistarr.length})`}
                      </p>
                      {showchecklistobj.hasOwnProperty(checklistobj._id) ? (
                        ...
                        <img
                          ...
                          alt=""
                          onClick={() =>
                            handleaddkey(checklistobj._id, "backlog")
                          }
                        />
                      )}
                    </div>
                        ...
                        return (
                          <div className={styles.inputcheck}>
                            <input
                              ...
                              onClick={() =>
                                updatemark(index, checklistobj._id)
                              }
                            />
                            <input type="text" value={checklist.data} />
                          </div>
                        );
                      })}
                    <div className={styles.datetype}>
                      ...
                      {checklistobj.year < currentyear && (
                        <button className={styles.selectedbtnred}>
                          {checklistobj.duedatestring
                            ? checklistobj.duedatestring
                            : ""}
                        </button>
                      )}
                      ...
                      {checklistobj.year === currentyear &&
                        checklistobj.month < currentmonth && (
                          <button className={styles.selectedbtnred}>
                            {checklistobj.duedatestring
                              ? checklistobj.duedatestring
                              : ""}
                      ...
                      <div>
                        <button
                          onClick={() =>
                            handlesectiontype("inprogress", checklistobj._id)
                          }
                        >
                          PROGRESS
                        </button>
                        <button
                          onClick={() =>
                            handlesectiontype("todo", checklistobj._id)
                          }
                        >
                          TO-DO
                        </button>
                        <button
                          onClick={() =>
                            handlesectiontype("done", checklistobj._id)
                          }
                        >
                          DONE
                        </button>
                  ...
                  <div className={styles.sectioncard}>
                    ...
                    <div className={styles.datetype}>
                      ...
                      <div>
                        <button
                          onClick={() =>
                            handlesectiontype("backlog", checklistobj._id)
                          }
                        >
                          BACKLOG
                        </button>
                    ...
                    <div className={styles.checklistclass}>
                      ...
                      {showchecklistobj.hasOwnProperty(checklistobj._id) ? (
                        ...
                        <img
                          ...
                          alt=""
                          onClick={() =>
                            handleaddkey(checklistobj._id, "inprogress")
                          }
                        />
                      )}
                    </div>
                ...
                return checklistobj.sectiontype === "done" ? (
                  <div className={styles.sectioncard}>
                    ...
                    <div className={styles.datetype}>
                      ...
                      {checklistobj.duedate !== "" && (
                        <button className={styles.selectedbtngreen}>
                          {checklistobj.duedatestring}
                        </button>
                      )}
                      <div>
      ...
      {deletecontainershow && (
        <Delete
          setdeletecontainershow={setdeletecontainershow}
      ...
      {Createcardshow && (
        <Createcard
          setCreatecardshow={setCreatecardshow}
          checklistchanged={checklistchanged}
          setchecklistchanged={setchecklistchanged}
        />
      )}
      {copied && <div className={styles.copied}>Link Copied</div>}
    </>
  );
};
