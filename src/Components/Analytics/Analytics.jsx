...
const Analytics = () => {
  ...
  useEffect(() => {
    ...
    if (filterchecklist === "Today") {
      // Adjust query based on filter setting
    ...
    axios
      ...
      .then((response) => {
        ...
        setduedatecount(
          tempduedatecount < 10 ? `0${tempduedatecount}` : tempduedatecount
        );
        setmoderatecount(
          tempmoderatecnt < 10 ? `0${tempmoderatecnt}` : tempmoderatecnt
        );
        setlowcount(templowcnt < 10 ? `0${templowcnt}` : templowcnt);
        sethighcount(temphighcnt < 10 ? `0${temphighcnt}` : temphighcnt);
        setbacklogcount(
          tempbackcount < 10 ? `0${tempbackcount}` : tempbackcount
        );
